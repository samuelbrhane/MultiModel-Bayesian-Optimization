...
with torch.no_grad():
    ...
    # Model predictions
    predictions_task0 = model_multi.likelihood(model_multi(candidates_task0)).mean.squeeze(-1).numpy()
    predictions_task1 = model_multi.likelihood(model_multi(candidates_task1)).mean.squeeze(-1).numpy()
...
candidates_df["MultiTaskModel_Activity"] = predictions_task0_original
candidates_df["MultiTaskModel_Stability"] = predictions_task1_original

# Save raw candidates with predictions
now = datetime.now()
candidates_df.to_excel(
    os.path.join(optimization_output_dir, f"Next_candidates_MultiTaskGP_original_{now.strftime('%Y%m%d_%H-%M-%S')}.xlsx"),
    index=False
)

# Match candidates to closest compositions in reduced composition space
candidates_selection = pd.DataFrame(columns=elements + ["MultiTaskModel_Activity", "MultiTaskModel_Stability"],
...
candidates_selection.to_excel(
    os.path.join(optimization_output_dir, f"Next_candidates_MultiTaskGP_{now.strftime('%Y%m%d_%H-%M-%S')}.xlsx"),
    index=False
)
